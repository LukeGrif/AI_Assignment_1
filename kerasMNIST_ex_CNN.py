...
model = Sequential([
    ...
    Conv2D(32, kernel_size=3, padding='same', input_shape=training_images.shape[1:]),  # Increase the number of filters
    Activation('relu'),
    MaxPooling2D(pool_size=(2, 2), strides=(2, 2)),
    Dropout(0.25),  # Dropout after first conv block

    # Second Convolutional Block (New)
    Conv2D(64, kernel_size=3, padding='same'),  # More filters
    Activation('relu'),
    MaxPooling2D(pool_size=(2, 2), strides=(2, 2)),
    ...
    Dense(128),  # Increase the units in the dense layer
    Activation('relu'),
    Dropout(0.5),  # Dropout after dense layer

    # Output Layer
    Dense(10),  # 10 output classes for MNIST
    Activation('softmax')
])
